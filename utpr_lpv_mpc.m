% Authors: donghao
% Date: 2024-08-27
% Date: 2024-08-27 test MPC solve time 
...
%%
N = 100;
Q = diag([2000, 1000, 1000, 2000, 100, 50]);
R = 1*diag([10, 8]);
...
for k = 1:Nsim
    ...
    a = -0.003;
    b = 0.003;
    AA = [ (a + ( b - a ) * rand(1,1));0;0;zeros(3,1)];

    if (400 <= k) && ( k <= 480)
        x_mpc(:,k+1) = x_mpc(:,k+1) + AA;
    end
...
for k = 1:Nsim+1
    ee_pos(:, k) = utpr_xy_position(x_mpc(:,k));
end

ee_pos = ee_pos';
datapos = [x_mpc(1:3,:)', ee_pos];
xlsdata = ["joint_pos1", "joint_pos2", "joint_pos3", "x", "y"; datapos ];
xlswrite("E:\Code\TriplePendulum\m_utpr_lpv_mpc\Matlab\UTPR_mpc_result.xlsx", xlsdata);