...
#define REQUEST_RETRIES  5   //  Number of tries before we abandon
...
void sendRequest_helper(void *zm, const double input_data[], unsigned int size )
{
    if(size > MAX_SIZE){
        std::cerr << " Error: size exceeds MAX_SIZE " << std::endl;
        return;
    }

    std::array<double, MAX_SIZE> Vec; 
    for (int i = 0; i < size; i++){
        Vec[i] = input_data[i];
    }
    
    // Vec[size] = static_cast<double>(iter);

    std::string request_str;
    encode_double_data(Vec, size, request_str);
...
{
    std::cout << "Creating ZmqMgr with connection string: " << connStr << std::endl;
    ZmqMgr* zmqMgr = new ZmqMgr(connStr);
    if (zmqMgr == nullptr)
    {
        std::cout << "Failed to allocate memory for ZmqMgr" << std::endl;
    }
    else
    {
        std::cout << "Successfully created ZmqMgr: " << zmqMgr << std::endl;
    }
    return reinterpret_cast<void *>(zmqMgr);
}
...
void update_wrapper(void *zm, unsigned int *iter_ptr, const double input_data[], const unsigned int size )
{
    ...
    // static unsigned int last_iter = 0;
    // if(((*iter_ptr) > last_iter) && (!sendmessage)){
    //     sendRequest_helper(zm, input_data, size);
    //     sendmessage = true;
    // } 
    // last_iter = (*iter_ptr);
