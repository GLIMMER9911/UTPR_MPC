function ee = utpr_xy_position(state, spec)
...
% l_1  = 0.34;  % Length of the arm
% l_2  = 0.29;   % Length of the pendulum
% l_3  = 0.52;   % Length of the pendulum
...
x(2) = -l_1 * sin(state(1));
y(2) = l_1 * cos(state(1));
x(3) = x(2) - l_2 * sin(state(2)+state(1));
y(3) = y(2) + l_2 * cos(state(2)+state(1));
x(4) = x(3) - l_3 * sin(state(3)+state(2)+state(1));
y(4) = y(3) + l_3 * cos(state(3)+state(2)+state(1));
